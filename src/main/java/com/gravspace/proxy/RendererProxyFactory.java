...
public class RendererProxyFactory {
...
//			task_args.add(Arrays.asList(args));
	...
	public static <T> T getProxy(Class<T> iface, Class<? extends IPersistanceAccessor> concrete, ConcurrantCallable caller) {
		return (T) Proxy.newProxyInstance(
				RendererProxyFactory.class.getClassLoader(), new Class[] { iface },
				new RendererProxyImpl(caller, concrete.getCanonicalName()));
	}
	
	@SuppressWarnings("unchecked")
	public static IRenderer getDefaultRender(ConcurrantCallable caller) {
		return (IRenderer) Proxy.newProxyInstance(
				RendererProxyFactory.class.getClassLoader(), new Class[] { IRenderer.class },
